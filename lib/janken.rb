class Player
  attr_reader :name
  attr_accessor :score, :janken_count, :win_count, :bonus_once
...
class Game
  def initialize(count, players)
    @players = players
    @jankens = Array.new(count) { Janken.new(*players) }
  ...
  def play
    ...
    @jankens.each.with_index(1) do |janken ,n|
      result << "<#{n}回目>\n"
      result << "#{janken.play}\n"
      @draws << janken.draw?
      @players.each{ |player| player.janken_count += 1 }
  ...
  def three_times_draw_in_a_row?
    return false if @draws.count < 3

    @draws.each_cons(3).any?(&:all?)
  end
